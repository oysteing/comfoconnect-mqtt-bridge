...
class Mqtt:
    def __init__(self, sensor_topic, host, port, client_id, username, password, retain):
    ...
    def sensor_publish(self, sensor, value):
        ...
        asyncio.create_task(
            self.client.publish(
                self.sensor_topic + "/" + sensor.name.replace(" ", ""), to_mqtt_format(value), retain=self.retain
            )
        )
